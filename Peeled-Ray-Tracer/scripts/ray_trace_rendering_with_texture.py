...
class SolidObjects:
    ...
    def __init__(self,mesh,ambient=None,diffuse=None,specular=None,shininess=None,
                 reflection=0.0,refraction=0.0,ior=None,name=None,cast_shadow=True,texture=None):
        self.mesh = mesh
    ...
    def ray_intersection(self,ray_origin,ray_direction,layer):
        ...
        if len(distances)>0:
            ...
            if self.texture is None:
                ...
                bary = trimesh.triangles.points_to_barycentric(triangles=[self.mesh.triangles[nearest_face_index]], points=[loc])
                bary = bary[0]
                UV = self.mesh.visual.uv
                UV = ( UV-UV.min() ) / ( UV.max()-UV.min() )
                vert_uv = UV[self.mesh.faces[nearest_face_index]]
                uv_0 = vert_uv[0]
                uv_1 = vert_uv[1]
                uv_2 = vert_uv[2]
                loc_uv_x = uv_0[0]*bary[0] + uv_1[0]*bary[1] + uv_2[0]*bary[2]
                loc_uv_y = uv_0[1]*bary[0] + uv_1[1]*bary[1] + uv_2[1]*bary[2]
                # quering colors from texture map
                px = int(loc_uv_x*(self.texture.shape[0]-1))
                py = int(loc_uv_y*(self.texture.shape[0]-1))
                face_color = self.texture[py, px]
...
def nearest_intersected_object(objects, ray_origin, ray_direction,layer):
    ...
    for obj in objects:
        if obj is not None:
            values.append(obj.ray_intersection(ray_origin, ray_direction,layer))
